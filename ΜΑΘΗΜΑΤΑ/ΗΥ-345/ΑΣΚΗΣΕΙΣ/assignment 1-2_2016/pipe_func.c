...
void pipe_func(char *buffer, struct var *root_var){ 
	...
	while( buffer[i] != '\n' ) 
	{
		...
		if (guardian == 0) 
			{ exec_one[one_i] = buffer[i]; one_i++; }
		else if (guardian == 1) 
			{ exec_two[two_i] = buffer[i]; two_i++; }
	...
	k=0; while(exec_one[k] == ' '){ k++; }
	while(k>0){exec_one++;k--;}

	k=0; while(exec_two[k] == ' '){ k++; }
	while(k>0){exec_two++;k--;}

	i=0;
	while(exec_one[i] != '\n'){
		if(exec_one[i] == '\0'){exec_one[i] = ' ';}
		i++;
	}
	i=0;
	while(exec_two[i] != '\n'){
		if(exec_two[i] == '\0'){exec_two[i] = ' ';}
		i++;
	}
	...
	int check,pipefd[2];
	check = pipe(pipefd); 
	if (check == -1){ perror("pipe failed"); exit(1); }
