...
void redirection_func_type_two(char *buffer, struct var *root_var)
{
	...
	while (joker != NULL)
	{
		...
		if (k == 0)
		{
			if (i == 0)
			{
				strcpy(exec_one, joker);
				i++;
			}
			else
			{
				strcat(exec_one, " ");
				strcat(exec_one, joker);
			}
		}
		else
		{
			if (i == 0)
			{
				strcpy(file_to_redirect_stdout, joker);
				i++;
			}
			else
			{
	...
	{ // printf("pipe: I am the first child with pid = %d\n", (int) getpid());
		...
		FILE *ptr_file;
		ptr_file = fopen(file_to_redirect_stdout, "a");
