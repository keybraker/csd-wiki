...
int notValidAccess(char *name, int scope, int yylineno)
{
    ...
    while (currscope > 0)
    {
        ...
        if (reader != NULL && reader->scope == currscope)
        {
            break;
        }
        currscope--;
    ...
    while (reader != NULL && reader->scope >= 0)
    {
        ...
        while (readerS != NULL)
        {
            ...
            if (readerS->type == USERFUNC && readerS->isActive == TRUE && readerS->value.varVal->scope != 0 && readerS->value.varVal->scope != scope)
            {
                foundFunc = TRUE;
            }
...
{ // local rule is boolean , 1 is call from local rule 0 is not.
        ...
        else
        {
            ...
            while (scope >= 0)
            {
                ...
                if (reader != NULL)
                {
                    ...
                    if (foundFunc == TRUE && foundEqual == FALSE)
                    { // we didnt find var with same name and we found a function. So we insert the var
                        return NULL;
                    }
                    else if (foundFunc == FALSE && foundEqual == TRUE)
                    { // we found a var with same name but not a function in between, so we dont insert
                        return foundBucket;
                    }
...
int callExists(char *name, int scope)
{
    ...
    if (reader != NULL)
    {
        ...
        while (readerS != NULL)
        {
            if (readerS->union_t == function && strcmp(readerS->value.funcVal->name, name) == 0 && readerS->isActive == TRUE)
            {
                return TRUE;
            }
            if (readerS->union_t == variable && strcmp(readerS->value.varVal->name, name) == 0 && readerS->isActive == TRUE)
            {
                return TRUE;
            }
            readerS = readerS->nextScope;
...
int lookUpScopeRedeclaration(char *name, unionType type, int scope, int yylineno, RuleEnum rule)
{
    ...
    {
        reader = Scopelist;
        while (reader != NULL && reader->scope != scope - 1)
        {
            reader = reader->next;
        }
    }
        ...
        while (readerS != NULL)
        {
            ...
            if (type == function)
            {
                ...
                if (strcmp(readerS->value.funcVal->name, name) == 0 && readerS->union_t == function)
                {
                    ...
                    codeIsCorrect = FALSE;
                    printf(a_c_r "  Error: (l: %d) Redeclaration of '%s' as %s, but it has already been\n         decleared as %s ", yylineno, name, UnionTypeEnumStrings[type], UnionTypeEnumStrings[readerS->union_t]);
...
int changeActivity(int Scope)
{
    ...
    while (readerS != NULL && readerS->scope != Scope)
    {
