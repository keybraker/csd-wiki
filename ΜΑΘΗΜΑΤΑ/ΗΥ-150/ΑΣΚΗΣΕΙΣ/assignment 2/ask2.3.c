...
float funk(float px, float py, float kx, float ky){    // these are all floats because we want to be specific ex. 1.4313245
    
    float solu,num;
    
    if (px>kx){                                        // Finds which posibilities is the correct one
        if (py>ky){                                    // and executes the I arithmetical sentence.
    ...
    }else if(kx>px){                                   // There is a check for every possibility
        if (py>ky){
