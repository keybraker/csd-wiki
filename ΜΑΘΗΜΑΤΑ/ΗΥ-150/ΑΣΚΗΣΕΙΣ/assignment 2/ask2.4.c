...
int recusrsiveLinearSearch(std::vector<int> &zugoi, int place, int key, int k)
{
    ...
    if (place > k || key % 2 != 0)
    {              // Checks whether or not we have gone through the whole vector or the keyword is a odd.
        return -1; // If so, the function stops and returns -1 because the keyword did not match with any
    }              // element in the vetor
    if (zugoi[place] == key)
    { // Compares one element of the vector with the keyword
        return place;
    }
    else
    {                                                         // is the keyword is not found variable place increases by one
...
int main()
{
    ...
    k = 0;
    for (i = 0; i < 201; i++)
    { // making a loop that ends when i you reaches 201, because 200 is an even number
        if (i % 2 == 0)
        {                       // checks whether or not all i is even
