...
void insertInTree(struct present *R, int pid, int stock_cnt)
{
    ...
    if (R->lc != stock_sentinel && R->rc != stock_sentinel)
    {
        ...
        if (R->lc->pid > R->rc->pid)
        { // printf(" <%d> > <%d> ->",R->lc->pid,R->rc->pid);
            if (pid > R->lc->pid)
            { // printf(" ΠΑΩ ΑΡΙΣΤΕΡΑ ΤΟΥ <%d> ΣΤΟ <%d>  ->",R->pid,R->lc->pid);
                insertInTree(R->lc, pid, stock_cnt);
            }
            else if (pid > R->rc->pid)
            { // printf(" ΠΑΩ ΔΕΞΙΑ ΤΟΥ <%d> ΣΤΟ <%d> ->",R->pid,R->rc->pid);
            ...
            { // printf(" ΤΣΕΚΑΡΩ ->");
                ...
                { // printf(" ΠΑΩ ΔΕΞΙΑ ΚΑΙ ΤΕΡΜΑ ΑΡΙΣΤΕΡΑ ΤΟΥ <%d> ->",R->pid);
        ...
        { // printf(" <%d> > <%d> ->",R->lc->pid,R->rc->pid);
            ...
            { // printf(" ΤΣΕΚΑΡΩ ->");
                ...
                { // printf(" ΠΑΩ ΚΑΤΕΥΘΕΙΑΝ ΤΕΡΜΑ ΑΡΙΣΤΕΡΑ ΤΟΥ <%d> ->",R->pid);
                    left_all(R, pid, stock_cnt);
                    return;
                }
            }
    ...
    else if (R->lc == stock_sentinel && R->rc == stock_sentinel)
    { // printf(" ΟΠΟΥ ΘΕΛΩ, ΑΛΛΑ ΒΑΖΩ ΤΟ <%d> ΑΡΙΣΤΕΡΟ ΠΑΙΔΙ ΤΟΥ <%d>\n",pid,R->pid);
    ...
    else if (R->lc != stock_sentinel)
    { // printf(" ΜΟΝΟ ΤΟ lc ΥΠΑΡΧΕΙ ->");

        if (R->lc->pid < pid && R->lc->pid > R->pid)
        { // printf(" ΠΑΩ ΑΡΙΣΤΕΡΑ ΤΟΥ <%d> ΣΤΟ <%d> ->",R->pid,R->lc->pid);
            insertInTree(R->lc, pid, stock_cnt);
        }
        else
        { // printf(" ΒΑΖΩ ΤΟ <%d> ΔΕΞΙ ΠΑΙΔΙ ΤΟΥ <%d>\n",pid,R->pid);
    ...
    else if (R->rc != stock_sentinel)
    { // printf(" ΜΟΝΟ ΤΟ rc ΥΠΑΡΧΕΙ ->");

        if (R->rc->pid < pid && R->rc->pid > R->pid)
        { // printf(" ΠΑΩ ΔΕΞΙΑ ΤΟΥ <%d> ΣΤΟ <%d> ->",R->pid,R->lc->pid);
            insertInTree(R->rc, pid, stock_cnt);
        }
        else
        { // printf(" ΒΑΖΩ ΤΟ <%d> ΑΡΙΣΤΕΡΟ ΠΑΙΔΙ ΤΟΥ <%d>\n",pid,R->pid);
...
int buy_present(int pid, int stock_cnt)
{
    ...
    if (stock_tree == NULL)
    { // If there are no presents the first one is created
    ...
    { // If presents already exist in list,
