 ...
 ********************************************/
struct child *findLargerestSmaller_two(struct child *Age_categories, int cid)
{ // printf(" IN_LS ");if (cid == 135) { printf("IS 135\n"); }
    ...
    if (Age_categories->lc != NULL && Age_categories->rc != NULL)
    { // printf(" IN_ONE ");if (cid == 135) { printf("        IS 135\n"); }

        if (cid > Age_categories->lc->cid && cid > Age_categories->rc->cid)
        { // larger than both
        ...
        else if (cid > Age_categories->lc->cid && cid < Age_categories->rc->cid)
        { // larger than lc
            returner = findLargerestSmaller_two(Age_categories->lc, cid);
        }
        else if (cid < Age_categories->lc->cid && cid > Age_categories->rc->cid)
    ...
    else if (Age_categories->lc == NULL && Age_categories->rc == NULL)
    { // printf(" IN_TWO ");if (cid == 135) { printf("        IS 135->"); }

        return Age_categories;
    }
    else if (Age_categories->lc == NULL)
    { // printf(" IN_THREE ");if (cid == 135) { printf("        IS 135 ->"); }

        if (cid > Age_categories->rc->cid)
        { // if (cid == 135) { printf("        KISS 135\n"); }
            returner = findLargerestSmaller_two(Age_categories->rc, cid);
        }
        else
        { // if (cid == 135) { printf("        PISS 135\n"); }
            return Age_categories;
        }
    }
    else if (Age_categories->rc == NULL)
    { // printf(" IN_FOUR ");if (cid == 135) { printf("        IS 135\n"); }
...
void insert_child_in_Age_categories_two(struct child *Age_categories, int cid, int age, int did, int pid1, int pid2, int pid3)
{
    ...
    { // printf(" ---------------------------> FOUR ");// if (nd->lc != NULL && nd->rc != NULL)
...
struct present *right_left_most_new(struct present *AgeF, struct present *AgeR, int i)
{
    ...
    while (k == 0)
    {
        if (AgeR->lc == stock_sentinel && AgeR->rc == stock_sentinel)
        { // printf(" <-One ");
            if (i == 1)
            { // printf(" <-one ");
        ...
        else if (AgeR->lc != stock_sentinel && AgeR->rc != stock_sentinel)
        { // printf(" <-Two ");
            AgeF = AgeR;
            AgeR = AgeR->lc;
            i = 1;
        }
        else if (AgeR->rc != stock_sentinel)
        { // printf(" <-Three ");
            if (i == 1)
            { // printf(" <-one ");
                AgeF->lc = AgeR->rc;
        ...
        else if (AgeR->lc != stock_sentinel)
        { // printf(" <-Four ");
            AgeF = AgeR;
            AgeR = AgeR->lc;
            i = 1;
...
void delete_presents_with_stock_cnt_zero(struct present *Present)
{
    ...
    { // printf("Parent NULL - ");
        ...
        if (Present->lc == stock_sentinel && Present->rc == stock_sentinel)
        { // printf("one ");

            stock_tree = NULL;
        }
        else if (Present->lc != stock_sentinel && Present->rc != stock_sentinel)
        { // printf("two ");
        ...
        else if (Present->rc != stock_sentinel)
        { // printf("three ");

            Present->rc->parent = stock_sentinel;
            stock_tree = Present->rc;
        }
        else if (Present->lc != stock_sentinel)
        { // printf("four ");
