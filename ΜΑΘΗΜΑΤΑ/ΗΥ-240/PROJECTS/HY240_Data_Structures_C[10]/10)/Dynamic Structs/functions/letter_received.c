...
void check_correct(int i, int cid, int age, int did, int pid1, int pid2, int pid3)
{
    ...
    { //// ΘΕΛΕΙ ΔΟΥΛΕΙΑ ΕΝΑ ΚΟΠΙ ΠΑΣΤΑ ΕΙΝΑΙ
        ...
        while (reader != NULL)
        {
            ...
            if (reader->cid == cid)
            { // if present's id is found in the list than there is no reason to continue searching
            ...
            else
            {
                ...
                if (reader->cid < cid)
                { // cid is bigger so we go to the next block of the list

                    readerprev = reader;
                    reader = reader->next;

                    if (reader == NULL)
                    { // All presents in list have smaller ids than current
                ...
                else if (reader->cid > cid)
                { // current id is smaller than the one in the list
                    ...
                    if (readerprev == NULL)
                    { // if there is only one present and its id is bigger than current
                        newChild->next = Age_categories[i];
                        Age_categories[i] = newChild;
                    }
                    else
                    { // if the current cid is in the middle of two ids
